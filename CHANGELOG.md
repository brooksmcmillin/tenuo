...
- **Python `bool` arguments are now canonicalized as CBOR booleans (not
  integers).** `py_to_constraint_value` previously tried `i64` extraction
  before `bool`, and because Python's `bool` inherits from `int`, `True`
  and `False` silently became `ConstraintValue::Integer(1)` / `Integer(0)`
  on their way into the Rust core. The Rust side treated the same logical
  `true`/`false` as `ConstraintValue::Boolean`, giving a different CBOR
  encoding and therefore a different PoP digest across the FFI. Extraction
...
- **Split-view authorize APIs in `tenuo-core`**: `Warrant::authorize_with_pop_args`,
  ...
  (signed, covered by approval gates) and `constraint_args` (matched
  against the warrant's per-tool constraints). Exposed to Python via
  `authorize_one_with_pop_args` / `check_chain_with_pop_args`. The
  existing single-arg methods are thin wrappers that pass the same
  dict for both — no behavior change for non-transport callers.
- **`tenuo._pop_canonicalize.strip_none_values(args)`** — canonicalizer
  applied on both MCP sides before the Rust FFI boundary, so
  `Optional[...] = None` tool parameters no longer crash the
  canonicalizer.
- **Cross-language PoP canonicalization fixture**
  (`tenuo-core/tests/fixtures/pop_canonical.json`) pins the exact
  Ed25519 signature bytes that `Warrant::sign_with_timestamp` produces
  over a curated arg/key/warrant matrix. Asserted from Rust and Python;
  any future canonicalization drift is intentionally loud on one side
  and silent on the other. Regenerator lives at
  `tenuo-python/tests/unit/_regen_pop_canonical.py`.
- **MCP CI smoke lane** — new `MCP Smoke (FastMCP + SecureMCPClient)`
  job runs `test_mcp_ci_smoke.py`, a stdio subprocess test covering
  allow, deny, and `None`-arg behavior on real transport wiring, next
  to the in-process local-vs-server parity matrix in
  `tests/adapters/test_mcp_local_server_parity.py`.
- **Invariant test infrastructure** for cross-adapter drift — three
  permanent contracts replace the ad-hoc deep-review loops:
  `tests/adapters/test_boundary_contract.py` (every Tenuo exception
  with `error_code` must wrap to non-retryable `ApplicationError` with
  `type=error_code` and `__cause__` preserved — new exception classes
  join the sweep automatically),
  `tests/adapters/test_adapter_audit_invariant.py` (audit callbacks in
  Temporal / OpenAI / CrewAI must never re-raise, and must log
  failures with `exc_info=True`), and
  `tests/property/test_bounded_collections.py` (every long-lived cache
  in the Temporal integration stays within its documented bound under
  Hypothesis workloads). As a corollary, `tenuo.openai` and
  `tenuo.crewai` now log audit-callback failures with `exc_info=True`
  so operators see a traceback from every adapter.

### Fixed

- **Temporal tenant isolation in multi-namespace / multi-worker
  processes.** Two related defects could leak capabilities or signing
  keys across tenants when one Python process hosted multiple
  Temporal clients/workers:
  - Workflow-internal stores (`_workflow_headers_store`,
    `_workflow_config_store`, `_pending_activity_fn`,
    `_pending_activity_approvals`) were keyed by
    `workflow.info().workflow_id`, which is only unique per namespace.
    Two tenants sharing a workflow name would clobber each other's
    entries and cross-attach headers. They are now keyed by
    `workflow.info().run_id` (globally unique). The client-side
    `_headers_by_workflow_id` stays keyed by `workflow_id` (the only
    key available pre-run), but no longer mirrors into the
    run-scoped store.
  - The module-level `_worker_config` slot was last-write-wins across
    worker registrations, so Worker A's `_tenuo_internal_mint_activity`
    could sign child warrants with Worker B's key resolver. Replaced
...
- **`TenuoClientInterceptor.set_headers_for_workflow` is bounded**
  ...
  memory. Added `discard_headers_for_workflow(workflow_id)` for
  explicit cleanup.
- **Audit-callback failures log with a traceback** on both ALLOW and
  DENY paths (`exc_info=True`), matching the control-plane sink.
  DENY-path audit loss is compliance-critical; operators can now
  diagnose the callback bug and recover the dropped record.
- **`set_activity_approvals` warns on overwrite** — back-to-back
  calls without an intervening `execute_activity` log a warning
  naming the `asyncio.gather` footgun (only the first dispatch would
  receive approvals). The docstring spells out the one-shot semantics.
- **`KeyResolver` errors normalize to `KeyResolutionError`** across
  `Vault`, `AWSSecretsManager`, `GCPSecretManager`, and the
  `EnvKeyResolver` decode path — every failure path (transport,
  permissions, decode) surfaces `KeyResolutionError from e`, so
  `__cause__` is preserved and callers have a single documented class
  to handle.
- **`SigningKey.__repr__` / `__str__` are explicitly redacted.**
  `repr(sk)` returns `SigningKey(public_key=<4-byte prefix>…, secret=[REDACTED])`
  instead of the default `<builtins.SigningKey object at 0x…>`,
  preventing secret-bytes leakage through `logger.info(f"{sk}")`,
  `ApplicationError(str(sk))` in workers, or pytest failure repr
  dumps. Different keys remain distinguishable via the public-key
  prefix.
- **Other Temporal correctness fixes.** `trusted_roots_provider`
  refresh now routes through `_build_authorizer` so
  `clearance_requirements` and the current SRL survive key rotation.
  `TenuoWorkerInterceptor(cfg)` with only `trusted_roots_provider=`
  no longer raises `ConfigurationError` when it installs the default
  control plane (shallow-copies the config instead of
  `dataclasses.replace`, which would re-run `__post_init__`).
  Signal/update denial events log the real warrant id (decoded from
  the stored header) with `<no-warrant>` / `<undecodable-warrant>`
  sentinels for edge cases. `TenuoMetrics._latencies` is a bounded
  ring (1024 samples). A malformed `x-tenuo-pop` header surfaces as
  non-retryable `PopVerificationError` instead of a raw
  `binascii.Error`. `authorized_signals=[]` /
  `authorized_updates=[]` (a deny-everything footgun) is rejected at
  config time; use `None` to disable the allowlist.
  `emit_for_enforcement` honors `redact_args_in_logs=True` on both
  paths so arg values aren't leaked off-host.
  `TenuoTemporalPlugin` now works on a copy of the user's config so
  two workers sharing one config stay isolated.
- **Temporal sandbox / config hardening.** Tenuo's domain exceptions
  are registered as `workflow_failure_exception_types` on supporting
  SDKs. `preload_all()` failures log at `ERROR`; `EnvKeyResolver`
  raises `ConfigurationError` instead of relying on an `os.environ`
  fallback (which is blocked inside the sandbox);
  `ensure_tenuo_workflow_runner` warns for `UnsandboxedWorkflowRunner`
  and unknown custom runners; the duplicate-registration error points
  at `Client.connect(plugins=[plugin])`.
- **Temporal polish and docs.** Removed placeholder config fields
  (`signal_constraints`, `update_constraints`, `enable_tracing`) and
  dead helpers (`_compute_pop_challenge`, `POP_WINDOW_SECONDS`,
  `_MintRequest.holder_bytes` / `clearance_bytes`). Demoted the
  interceptor's startup `INFO` log to `DEBUG`.
  `TENUO_TEMPORAL_SIMPLE_PLUGIN_NAME` is an alias of
  `TENUO_TEMPORAL_PLUGIN_ID`. Fixed broken doc anchors, a stale
  Python-version note, a misleading `TenuoWarrantContextPropagator`
  docstring, a `docs/enforcement.md` rename regression, and the
  `examples/temporal/README.md` `activity_fns` guidance (the plugin
  auto-discovers from `Worker(activities=...)`). Runnable
  `quickstart.md` and `approvals.md` Temporal snippets now import
  `Client` and define `resolver`. Added three operational subsections
  to `docs/temporal-reference.md`: warrant TTL vs. workflow lifetime,
  Temporal event-history overhead (with a worked 200-activity example;
  `warrant_hash` + LRU cache is flagged as planned for v0.2), and
  `KeyResolver` ↔ workflow-sandbox compatibility.
- **MCP PoP parity across config asymmetry.** A client without a
  `CompiledMcpConfig` loaded (or with a different one) can now call a
  server that has a config; PoP byte parity no longer depends on the
  extraction schema. Signature-verification denials now only mean
  what they should: the caller is not the legitimate holder of the
  warrant.
- **`None` tool-call arguments no longer crash signing / verification.**
  Both MCP client and verifier apply `strip_none_values` symmetrically
  before the FFI boundary. `enforce_tool_call_async` (used by
  `SecureMCPClient(..., warrant_context=True)`) canonicalizes the
  same way before approval-gate evaluation, PoP signing, and
  authorizer checks.
- **Removed misleading "unauthenticated arguments" server warning.**
  Under split-view authorize, every raw wire arg is covered by the
  PoP signature regardless of the extraction mapping.
...
- Upgrade client and server together. A pre-upgrade client talking to a
  ...
  common case where client and server have identical configs loaded **and**
  no tool arguments are booleans, but will otherwise silently fail because
  the signed bytes differ under one of the two canonicalization changes
  in this release. If you cannot upgrade both sides simultaneously, pin
  the `tenuo-python` / `tenuo-core` versions on each side and plan a
  coordinated rollout.
